...
# Disable the crash reporter and use notifications
...
# Installing settings and theme for iTerm
ITERM_THEMES=$DOTFILES_DIR/iterm/*.itermcolors

defaults write com.googlecode.iterm2 "PrefsCustomFolder" -string "${DOTFILES_DIR}/iterm"
defaults write com.googlecode.iterm2 "LoadPrefsFromCustomFolder" -bool true

for f in $ITERM_THEMES; do
  THEME=$(basename "$f")
  defaults write com.googlecode.iterm2 'Custom Color Presets' -dict-add "$THEME" "$(cat "$f")"
done
