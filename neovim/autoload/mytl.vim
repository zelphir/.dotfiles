...
function! mytl#update()
  ...
  for buffer_id in range(1, largest_buffer_id)
    ...
    let buffer.tail = buffer.not_new ? buffer_tail : g:mytl_new_buffer_name
  ...
  if !g:mytl_always_show_tabline && len(s:buffer_ids) == 1 && tabpagenr("$") == 1
    set showtabline=0
  endif
endfunction
...
function! s:GetBufferElements(capacity, buffer_padding)
  ...
  if trunced_right > 0
    ...
    let right_trunc_elem.value = trunced_right . " " . g:mytl_right_trunc_icon
...
function! s:IsBufferElement(element)
  if index(g:mytl_buffer_types, a:element.type) >= 0
    return 1
  endif
...
function! s:GetTypeHighlight(type)
  return "%#" . g:mytl_prefix . a:type . "#"
endfunction

function! s:Render()
  let sep_len = s:Len(g:mytl_separator)

  let tabs_count = tabpagenr("$")
  let tabs_len = (1 + s:Len(g:mytl_tab_icon) + 1 + sep_len) * tabs_count

  let left_trunc_len = 1 + s:Len(g:mytl_left_trunc_icon) + 1 + 2 + 1 + sep_len
  let right_trunc_len =  1 + 2 + 1 + s:Len(g:mytl_right_trunc_icon) + 1 + sep_len
  let trunc_len = left_trunc_len + right_trunc_len

  let capacity = &columns - tabs_len - trunc_len - 5
  let buffer_padding = 1 + (g:mytl_use_devicons ? 1+1 : 0) + 1 + sep_len
  ...
  for i in range(0, len(elements) - 2)
    ...
    let icon = ""
    if g:mytl_use_devicons && s:IsBufferElement(elem)
      let icon = " " . WebDevIconsGetFileTypeSymbol(elem.value)
    elseif elem.type == "Tab"
      let icon = " " . g:mytl_tab_icon
    endif
    ...
    if s:IsBufferElement(elem)
      if elem.is_modified && g:mytl_modified_icon != ""
        let render = render . g:mytl_modified_icon
      endif
    endif

    let render = render . " "

    let separator =  g:mytl_has_separator[left.type][right.type]
...
function! mytl#bwipe(bang, buffer)
  let btarget = s:GetBuffer(a:buffer)

  let filters = get(g:, "mytl_bwipe_filters", [])
...
function! mytl#bonly(bang, buffer)
  let btarget = s:GetBuffer(a:buffer)
